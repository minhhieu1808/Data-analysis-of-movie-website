...
const MediaItem = ({ media, mediaType, isSearched}) => { 
  ...
  function tracking(){
    if(isSearched == true){
      ...
      trackSelfDescribingEvent({
      event: {
      schema: 'iglu:com.example/view_event/jsonschema/1-0-0',
      data: {
            isSearched: isSearched,
            mediaId: media.id,
            mediaTitle: media.title || media.name,
  ...
  return (
    <Link to={mediaType !== "people" ? routesGen.mediaDetail(mediaType, media.mediaId || media.id) : routesGen.person(media.id)} onClick={tracking}>
      <Box sx={{
